...
// Global variables to track the state of clicks and positions
LineCollection lineSet;
bool isFirstClick = true;    // To track whether it's the first or second click
glm::vec3 startPoint;        // To store the start point of a line
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
    {
        ...
        if (isFirstClick)
        {
            startPoint = glm::vec3(xpos, ypos, 0.0f);
            isFirstClick = false;
        }
        else
        {
            glm::vec3 endPoint = glm::vec3(xpos, ypos, 0.0f);
            if (lineSet.size < MAX_LINES)
            {
                Line line(startPoint, endPoint, lightGreen);
                line.setup();
